import Num from '@/components/common/font/Font';

export interface QAItemData {
  id: string;
  question: string;
  answer: string;
}

export interface SelfIntroduction {
  content: React.ReactNode;
}

/**
 * メッセージデータ - 自己紹介
 */
export const SELF_INTRODUCTION: SelfIntroduction = {
  content: (
    <>
      <p className='mb-6'>
        初めましてAIと申します
        <br />
        簡単ですが自己紹介をさせてください。
      </p>
      <p className='mb-6'>
        私はフロントエンドエンジニアとしてSES企業で現在働いております。
        <br />
        <Num>5</Num>
        年程化粧品業界で接客から営業、企画を経て現在IT業界へ転職し <Num>5</Num>
        年目になりました。（育児休暇を除く）
        <br />
        そして子供が生まれ、フルリモートを現状していた事もあり意を決して2年前に北海道に移住を決意！
        お陰様で子供とかまくらを作ったり広い公園で遊んだりと、非常に充実した生活を送っております。
      </p>
    </>
  ),
};

/**
 * メッセージデータ - QA項目
 */
export const QAITEMS: QAItemData[] = [
  {
    id: 'career-change',
    question: '化粧品業界から何故エンジニアへの転職をしたのか？',
    answer:
      '自分が担当になった商品が＠コスメによって大ヒットし、売上が急増した事でネットでの宣伝の重要性を痛感しました。それから独学でHTML/CSS/JavaScriptを学び、プログラミングの楽しさに目覚め、もっと深く学びたいと思い、転職を決意しました。',
  },
  {
    id: 'work-style',
    question: '1日PCに向かう仕事になって辛い？',
    answer:
      'お客様との接客はとても大好きですが、ゲームが大好きな事もあり1日15時間ぶっ通しでやっていた経験もある為、画面を見続けて作業する事にさほど抵抗はありませんでした。',
  },
  {
    id: 'hokkaido-move',
    question: '北海道への移住',
    answer:
      '仕事で1回しか言った事なく無謀だと皆からいわれました（笑）私自身仕事も出来なくなるのでは？と心配もありましたが、近年リモートの敷居も下がり、子育てしやすい環境に住みたいという思いが強くなり主人の地元である北海道の移住へ踏み切りました！',
  },
];
